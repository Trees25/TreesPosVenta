...
export function FormatearNumeroDinero(numero, currency, iso) {
    if (currency === undefined || numero === undefined) {
        return "";
    }
    const esiso = "es-" + (iso || "AR");
    const numeroconvertido = Number(numero).toLocaleString(esiso, {
        style: "currency",
        currency: `${currency}`,
...
export function numeroALetras(num, nombreMoneda = "PESOS") {
    if (!num && num !== 0) return "";
